...
import os
import sys
sys.path.append(os.getcwd() + '/Code/DataCode/')
...
def data_load_synthetic(data_folder = "wpbc", type = "variable_p", p_available = 0.75, if_base_feat = False, n_base_feat = 0):
    ...
    if data_folder == "wpbc":
        ...
        if type == "variable_p" and if_base_feat:
            X_base = X[:,:n_base_feat]
            X_haphazard = X[:, n_base_feat:]
            mask = (np.random.random((number_of_instances, n_feat-n_base_feat)) < p_available).astype(float)
            mask[:,32-n_base_feat][np.where(np.isnan(X[:,32-n_base_feat]))] = 0
            mask = check_mask_each_instance(mask)
            X_haphazard = np.where(mask, X_haphazard, 0)
            return X, Y, X_base, X_haphazard, mask
    ...
    if data_folder == "wbc":
        ...
        if type == "variable_p" and if_base_feat:
            ...
            X_haphazard = X[:, n_base_feat:]
            mask = (np.random.random((number_of_instances, n_feat-n_base_feat)) < p_available).astype(float)
            mask[:,5-n_base_feat][np.where(np.isnan(X[:,5-n_base_feat]))] = 0
            mask = check_mask_each_instance(mask)
            X_haphazard = np.where(mask, X_haphazard, 0)
            return X, Y, X_base, X_haphazard, mask
